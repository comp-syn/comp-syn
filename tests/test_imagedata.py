...
TEST_IMAGES = Path(__file__).parent.joinpath("test-assets/test-downloads/raw-images/atlantis/known-dist/pytester/testoclock/").resolve()
...
def test_load_image_dict_from_folder() -> None:
    ...
    count = 0
    for rgb_vector, jzazbz_vector in zip(image_data.rgb_dict["atlantis"], image_data.jzazbz_dict["atlantis"]):
        count += 1
